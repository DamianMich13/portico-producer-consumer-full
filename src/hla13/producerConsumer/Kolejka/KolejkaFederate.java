...
public class KolejkaFederate {
    ...
    public void runFederate() throws Exception {
        ...
        while (fedamb.running) {
            ...
            // Wywalanie zniecierpliwionych klientow
            if (fedamb.klienci.size() > 0) {
                ArrayList<Klient> klientsToRemove = new ArrayList<>();
                for (Klient klient : fedamb.klienci) {

                    // Jezeli jest niecierpliwy
                    if (klient.getImpatientTime() > 0.0) {

                        // Minal czas zniecierpliwienia klienta
                        if (fedamb.federateTime > klient.getImpatientTime()) {
                            klientsToRemove.add(klient);
                        }
                    }
                }

                for (Klient klient : klientsToRemove){
                    fedamb.klienci.remove(klient);
                    log("Usunieto z kolejki klienta o id: " + klient.getKlientId() + ", w czasie: " + fedamb.federateTime);
                }
            }


            while (tableStock > 0 && !fedamb.klienci.isEmpty() && fedamb.grantedTime == timeToAdvance) {
                timeToAdvance += fedamb.federateLookahead;
    ...
    private void sendInteraction(double timeStep) throws RTIexception {
        ...
        byte[] id = EncodingHelpers.encodeInt(fedamb.klienci.get(0).getKlientId());
        fedamb.klienci.remove(0);
